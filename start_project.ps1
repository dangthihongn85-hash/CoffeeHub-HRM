...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'DANG CHAY BACKEND SPRING BOOT (CONG 8080)...' -ForegroundColor Cyan; cd '$PSScriptRoot\hrm-backend'; .\mvnw spring-boot:run"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'DANG CHAY FRONTEND ANGULAR (CONG 4200)...' -ForegroundColor Cyan; cd '$PSScriptRoot\hrm-frontend'; `$env:NG_CLI_ANALYTICS='false'; npm start"
...
Write-Host "Ban co the dong cua so chinh nay." -ForegroundColor Gray
